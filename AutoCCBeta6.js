/*
 * ---SynthWorks AutoCC Beta6 by Tom Xi and Brian Li
...
function resetBool(e, fTriggerMenu, fTrigger, fTrigMode, fResetTrig) {
/*Return True or False depending on Trigger conditions.
  fTriggerMenu: a list of strings of the note names
  fTrigger: MIDI note values
  fTrigMod: [Once Only, Every Time]*/
    tNote = MIDI.noteNumber(fTriggerMenu[fTrigger]);
    if (fResetTrig = 1) { //"Natural Termination"
        return false;
    }
    switch (fTrigMode) {
        case 0: //once only
            if (e instanceof NoteOff && e.pitch == tNote && onetimeThrough) {
                onetimeThrough = false;
                return true;
            } else {
                return false;
        ...
        case 1: //EveryTime
            if (e instanceof NoteOff && e.pitch == tNote) {
                return true;
            } else {
                return false;
            }
...
function HandleMIDI(e) {
    ...
    var reset = resetBool(e, mTrigger, gpTrigger, gpTrigMode, gpResetTrig);
    ...
    if (reset) {
        CCIter = 0;
        processSwitch = false;
        sendCC(ch, gpTarget, 0)
        Trace("Interrupted. It took " + (new Date() - startTime) + " ms.");
    }
}
...
function ParameterChanged(param, value) {
    ...
    gpResetTrig = GetParameter(pResetTrig);
...
var mCh = buildChMenu();
var mResetTrig = ["Natural Termination", "Trigger Note NoteOff"];
...
PluginParameters.push({name: pResetTrig,    //10
                       type: "menu",
               valueStrings: mResetTrig,
                   minValue: 0,
                   maxValue: 1,
               defaultValue: 0,
              numberOfSteps: 1});